...
describe("EventTarget.events", () => {
  ...
  it("should contain a list of events with the right lengths", () => {
    expect(events(element, "click")).toHaveLength(2);
    expect(events(element, "blur")).toHaveLength(1);
    expect(events(element, "focus")).toHaveLength(1);
    expect(events(element, "dblclick")).toHaveLength(0);
  });
});