...
> extends Internal_Node implements Crafty.Element<N, T, P> {
  ...
  txt(fnOrText?: ((origin: string) => string) | string): string | void {
    ...
    this.children = this.children.filter(n => !(n instanceof CText));

    this.children.push(new CText(next));
  }
...
> extends Internal_Element<"html", T, P> implements Crafty.HTMLElement<T, P> {
  ...
  constructor(tag: T, props?: P, chilren?: Crafty.Children) {
    super("html", tag, props, chilren);
  }
...
export class Internal_VoidHTMLElement<T extends HTMLTag, P extends Crafty.Props<T>> extends Internal_HTMLElement<T, P> implements Crafty.VoidHTMLElement<T, P> {
  append(_: Crafty.Node): never {
    throw new Internal_ChildrenNotAllowedException("VoidHTMLElements are auto closing elements and therfore cannot have children");
  }
